...
ENDPOINT = 'http://test.datacite.org/mds'
...
response = endpoint['metadata'].post(metadata,
                                     content_type: 'application/xml;charset=UTF-8') do |response, request, result, &block|
...
response = endpoint['doi'].post("doi=#{doi}\nurl=#{url}",
                                content_type: 'text/plain;charset=UTF-8') do |response, request, result, &block|
  if [301, 302, 307].include? response.code
    response.follow_redirection(request, result, &block)
  else
    response.return!(request, result, &block)
